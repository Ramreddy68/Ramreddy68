<!-- ================= HEADER ================= -->
<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=260&section=header&text=Seetharam%20🚀&fontSize=50&fontAlignY=38&animation=twinkling&fontColor=ffffff"/>

### DevSecOps Engineer | Cloud Architect | Kubernetes | CI/CD | IaC Specialist

<p>
<a href="https://www.linkedin.com/in/ram-reddy-srr/">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin"/>
</a>
<a href="https://github.com/Ramreddy68">
<img src="https://img.shields.io/badge/GitHub-Follow-000000?style=for-the-badge&logo=github"/>
</a>
</p>

<img src="https://komarev.com/ghpvc/?username=Ramreddy68&style=for-the-badge&color=blueviolet"/>

</div>

---

<!-- ================= ANIMATED TYPING ================= -->
<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=00F7FF&center=true&vCenter=true&width=800&lines=DevSecOps+Engineer;Cloud+%7C+AWS+%7C+Kubernetes+%7C+Terraform;CI%2FCD+Automation+Expert;Security+First+Infrastructure+Builder;GitOps+%7C+ArgoCD+Specialist" />

</div>

---

# 🧠 About Me

Experienced **DevSecOps Engineer** focused on building **secure, scalable, and automated cloud platforms**.

I specialize in bridging **Development + Security + Operations** using modern cloud-native technologies.

- ☁️ AWS Cloud Architecture & Design  
- 🔐 DevSecOps pipeline implementation (SAST, DAST, SCA)  
- ⚙️ Infrastructure as Code with Terraform & Ansible  
- 🚀 GitOps workflows using ArgoCD  
- ☸️ Kubernetes cluster deployment & management  
- 🐳 Containerized microservices with Docker  
- 📈 Observability & monitoring (Prometheus/Grafana concepts)

---

# 🛠️ Tech Stack

## ☁️ Cloud
![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonaws)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux)

## 🚀 DevOps
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge)
![ArgoCD](https://img.shields.io/badge/ArgoCD-EF7B4D?style=for-the-badge)

## ☸️ Kubernetes
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=for-the-badge)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge)

## 🏗️ IaC
![Terraform](https://img.shields.io/badge/Terraform-623CE4?style=for-the-badge)
![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=for-the-badge)

## 🔐 Security
![SonarQube](https://img.shields.io/badge/SonarQube-4E9BCD?style=for-the-badge)
![Trivy](https://img.shields.io/badge/Trivy-1904DA?style=for-the-badge)
![OWASP](https://img.shields.io/badge/OWASP-000000?style=for-the-badge)

## 💻 Scripting
![Python](https://img.shields.io/badge/Python-FFD43B?style=for-the-badge)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge)

---

# 🚀 Core Expertise

- Cloud Infrastructure Engineering (AWS)
- Kubernetes Cluster Architecture (EKS)
- CI/CD Pipeline Design & Automation
- DevSecOps Integration (Shift-left security)
- Infrastructure as Code (Terraform, Ansible)
- GitOps-based deployments (ArgoCD)
- Microservices container orchestration
- Linux system automation & scripting

---

# 🔥 Featured Projects

## ☸️ GitOps Kubernetes Platform
- ArgoCD-based deployment automation
- Fully version-controlled infrastructure (GitOps)

## 🔐 Secure CI/CD Pipeline
- Integrated SonarQube + Trivy scanning
- Automated security gates in deployment pipeline

## ☁️ AWS Infrastructure Automation
- Terraform-based modular infrastructure provisioning
- Scalable and reusable cloud architecture

## 🧩 Microservices Platform
- Dockerized applications deployed on Kubernetes
- Load balancing, scaling & service discovery

---

# 📊 GitHub Analytics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=Ramreddy68&show_icons=true&theme=tokyonight&hide_border=true" height="170"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ramreddy68&layout=compact&theme=tokyonight&hide_border=true" height="170"/>

</div>

<div align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=Ramreddy68&theme=tokyonight&hide_border=true"/>

</div>

---

# 🎯 Engineering Philosophy

> Automate everything possible  
> Security is built-in, not added later  
> Infrastructure must be reproducible  
> Systems should self-heal and scale  
> Fail fast, recover faster

---

# 📜 Certifications
Certified Kubernetes Administrator (CKA)

---




  <p>DevSecOps Engineer | AWS | Kubernetes | Terraform</p>
  <p>Building secure & scalable cloud systems</p>

  <br>
  <a href="https://github.com/Ramreddy68">GitHub</a> |
  <a href="https://www.linkedin.com/in/ram-reddy-srr/">LinkedIn</a>
</div>

</body>
</html>
